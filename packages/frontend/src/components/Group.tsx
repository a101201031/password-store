...
import type { AccountModel, AccountGroupModel } from 'model';
...
interface GroupAddFormTypes extends Pick<AccountGroupModel, 'group_name'> {}

function GroupAdd() {
  const accountList = useRecoilValue(accountListSltr);
  const { control, handleSubmit } = useForm<GroupAddFormTypes>();

  const onSubmit: SubmitHandler<GroupAddFormTypes> = (data) => {};
